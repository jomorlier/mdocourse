...
%% 
% What if we want to estimate the model at a new point  x* defined as 18 points 
% in [-1.5,0]. Please check also x*= 0.2 (extrapolation) !!
% 
% The regression problem is defined as follows:
% 
% Let $$\mathbf{x}_i \in {R}^{6}$ $ be an input vector and $$\mathbf{y}_i \in 
% {R}^{6}$$ be its corresponding target. The set of $$M$$ inputs are arranged 
...
% 
% 
% hey hey add something missing, we may need parameters to fit "a model" of 
% our covariance matrix. Let's try with a Standard Exponential (SE) Kernel.
% 
...
%% 
% but is there a way to find the $\theta_{optimal}$ ? oh $\theta = [l, \sigma_f, 
% \sigma_n]$
% 
...
% 
% where c is a constant and the matrix $$\mathbf{K}$ $is a function of the hyperparameters 
% (see equation k(xi,xj) = ...).
% 
% Based on the demo given on the GPML website, Let's try to implement this using 
% the GPML Matlab code is below.
% 
%